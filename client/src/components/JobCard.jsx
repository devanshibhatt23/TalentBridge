...
import { formatJobType, formatRelativeTime } from '../utils/formatters.js'
...
export function JobCard({ job }) {
  ...
  return (
    <Link to={`/jobs/${id}`} className="card list-card list-card--link">
      ...
      {job.createdAt ? <p className="list-card__time">Posted {formatRelativeTime(job.createdAt)}</p> : null}
      {job.skills?.length > 0 ? (
        <div className="tags">
